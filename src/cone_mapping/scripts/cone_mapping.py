...
class ConeMapper:
    ...
    def get_closer_cone(self, cone_x, cone_y):
        ...
        for idx, detected_cone in enumerate(self.detected_cones):
            ...
            distance = math.dist(
                [cone_x, cone_y], [detected_cone["x"], detected_cone["y"]]
            )
    ...
    def cone_callback(self, msg: Float32MultiArray):
        ...
        for i in range(0, len(msg.data), 4):
            ...
            closer_cone, cone_index, _ = self.get_closer_cone(cone_x, cone_y)
            ...
            if closer_cone is None:
                ...
                self.detected_cones.append(
                    {"x": cone_x, "y": cone_y, "colors": [color]}
                )
            # otherwise append the color detected this time to the closer cone
            else:
                rospy.loginfo(
                    "Close cone detected: {}".format(
                        (closer_cone["x"], closer_cone["y"])
                    )
                )
    ...
    def get_detected_cones(self):
        ...
        for detected_cone in self.detected_cones:
            ...
            rospy.loginfo(
                "Cone [{}] most frequent color: {}".format(
                    (detected_cone["x"], detected_cone["y"]), detected_color
                )
            )
